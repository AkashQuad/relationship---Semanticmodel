...
class SemanticModelRequest(BaseModel):
    ...
    relationships: List[Relationship]
    clone_report: bool = False
    # New Field: Allow user to specify report name dynamically
    report_name: Optional[str] = "Final_Sales_Report"
...
def upload_json_to_blob(data: dict, filename: str) -> str:
    ...
    log.info(f"Uploaded metadata JSON to: {filename}")
...
def create_semantic_model(request: SemanticModelRequest):
    try:
        ...
        target_workspace_id = request.target_workspace_id
        relationships = request.relationships
        # Use dynamic report name from request
        final_report_name = request.report_name

        log.info(f"Starting semantic model creation for report: {final_report_name}")
        token = get_auth_token()
        ...
        for r in relationships:
            ...
            if f_tbl not in blob_tables or t_tbl not in blob_tables:
                log.warning(f"Skipping relationship {f_tbl} -> {t_tbl} (table missing)")
                continue
        ...
        # Dataset Payload
        dataset_payload = {
            "name": f"{final_report_name}_DS", # Dynamic Dataset Name
            "defaultMode": "Push",
            "tables": [],
            "relationships": pbi_relationships,
        }
        ...
        if request.clone_report and TEMPLATE_REPORT_ID:
            ...
            clone_resp = requests.post(
                f"{POWERBI_API}/groups/{TEMPLATE_WORKSPACE_ID}/reports/{TEMPLATE_REPORT_ID}/Clone",
                headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"},
                # Use dynamic Report Name
                json={"name": final_report_name, "targetWorkspaceId": target_workspace_id, "targetModelId": dataset_id},
            )
            if clone_resp.ok: report_id = clone_resp.json()["id"]
